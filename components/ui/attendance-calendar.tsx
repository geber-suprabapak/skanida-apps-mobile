    ...
    (day: CalendarDay) => {
      try {
        ...
        if (!day?.isCurrentMonth) {
          ...
          console.log(
            `🚫 Day ${day?.date} not in current month (${monthName} ${currentYear}), ignoring press`,
          );
          return;
        }
          ...
          >
            {monthlyAttendance.loading ? (
              <View className="animate-spin">
                <RefreshCw size={16} color="#9ca3af" className="" />
              </View>
            ) : (
              <RefreshCw
                size={16}
        ...
        {monthlyAttendance.loading ? (
          ...
          <View key={`calendar-${currentYear}-${currentMonth}`}>
            {Array.from(
              ...
              (_, weekIndex) => (
                <View
                  key={`week-${currentYear}-${currentMonth}-${weekIndex}`}
                  className="flex-row"
                >
                  {(calendarDays || [])
